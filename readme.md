...
Porta автоматически создает SQLite базу данных `agents.db` для отслеживания:

- **Зарегистрированных агентов** - ID, имя, время создания
- **Истории операций** - все действия агентов с временными метками
- **Статистики использования** - количество операций, статус

### Автоматическое создание

База данных создается автоматически при первом запуске Porta:

```bash
# База данных будет создана автоматически
python3 porta.py
# INFO: База данных агентов инициализирована
```

### Исключение из версионирования

Файл `agents.db` исключен из git через `.gitignore`:

```bash
# База данных не отслеживается git
git status
# (agents.db не появится в списке изменений)
```

### Безопасное удаление

База данных может быть безопасно удалена - она пересоздастся автоматически:

```bash
# Удаление базы данных
rm agents.db

# При следующем запуске Porta база создастся заново
python3 porta.py
```
